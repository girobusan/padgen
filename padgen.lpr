...
uses {$IFDEF UNIX} {$IFDEF UseCThreads}
  ...
  SysUtils,
  CustApp,
  padgenlib { you can add units after this };
...
type
  ...
  begin
    ...
    if Pad = '' then
    begin
      Write('Can not generate pad. Check parameters and ');
      writeln('permissions (can you read /dev/random?)');
    end
    else
      writeln(Pad);
